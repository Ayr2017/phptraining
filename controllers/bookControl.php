<?php
  ...
  class Books extends DB 
  {
    ...
    function get()
    {
      ...
      $authorid = str_replace(",", "\" OR author_id LIKE \"", "$_GET[authorid]");
      $genreid =str_replace(",", "\" OR genre_id LIKE \"", "$_GET[genreid]");
      ...
      FROM (author JOIN author_has_books ON author_has_books.author_id = author.id ) JOIN books ON books.id=author_has_books.books_id 
        WHERE author_id LIKE $authorid GROUP BY book_id ) AS A 
